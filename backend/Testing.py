        ...
        for tickerIndex in range(total_num_tickers):
            ...
            for n in range(bars, total_length): # for the nth iteration, going through bars n-bars to n-1 
                ...
                for b in range(start, n):
                    if xSeq[0, tickerIndex, b, 1] > upper[b-zeroIndex] or xSeq[0, tickerIndex, b, 1] < lower[b-zeroIndex]:
                        terminate = True
                        break
                if terminate: continue
                for b in range(zeroIndex, start):
                    if xSeq[0, tickerIndex, b, 1] > upper[b-zeroIndex]:
                        totalLowerBound += (xSeq[0, tickerIndex, b, 1] - upper[b-zeroIndex]) ** 2
                    elif xSeq[0, tickerIndex, b, 1] < lower[b-zeroIndex]:
                        totalLowerBound += (xSeq[0, tickerIndex, b, 1] - lower[b-zeroIndex]) ** 2
                ...
                for i in range(bars):
                    ...
                    for j in range(max(0, i - r), min(bars - 1, i + r) + 1):
                        if i == 0 and j == 0:
                            c = xSeq[0, tickerIndex, zeroIndex, 1] - ySeq[0]
                            cost[k] = c * c
                            k += 1
                            continue
                        ...
                        d = xSeq[0, tickerIndex, i+zeroIndex, 1] - ySeq[j]
