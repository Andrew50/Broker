import numpy as np, tensorflow, os, datetime
...
def get_data(preprocessing_method,request):
	conn = redis.Redis(host='localhost', port=6379)
	classifications = []
	failed = 0
	for ticker, dt, classification in request:
		try:
			value = pickle.loads(conn.hget(tf+'screener',ticker))#get rid of dt becuase dont need for training
...
def compute_model(bar):
	preprocessing_method, model_type, setup_type = bar

	ds = data.get_ds(preprocessing_method)
	...
	for preprocessing_method in preprocessing_methods:
		for model_type in model_types:
			for setup_type in setup_types:
				args.append((preprocessing_method, model_type, setup_type))

	multiprocessing.Pool(4).map(compute_model, args)
