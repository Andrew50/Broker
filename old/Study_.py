...
class Screener:
	...
	def get(type = 'full', refresh = False, browser = None):
		...
		def get_full(refresh):
			...
			df3 = df3.reset_index()
			df3.to_feather("C:/dev/Broker/backend/scripts/sync/files/full_scan.feather")
			return df3['ticker'].tolist()

		def get_current(refresh,browser = None):
			if not refresh:
				try: return pd.read_feather("C:/dev/Broker/backend/scripts/sync/files/current_scan.feather")['ticker'].tolist(), browser
				except FileNotFoundError: pass
			try:
			...
			df.to_feather(r"C:/dev/Broker/backend/scripts/sync\files\current_scan.feather")
		...
		def get_intraday(browser = None):
			while True:
				try:
					...
					df = pd.read_feather("C:/dev/Broker/backend/scripts/sync/files/current_scan.feather")
					break
